...
def run_render(env: SnakeEnv, agent: A2CAgent, seed: int):
...
def run_eval(env: SnakeEnv, agent: A2CAgent, seeds: list[int]):
    results = []
    for s in seeds:
...
def main():
    ...
    ap.add_argument("--model", type=str, default="models/a2c_best.npz")
    ap.add_argument("--seed", type=int, default=0)
    ap.add_argument("--headless", action="store_true")
    ap.add_argument("--eval_episodes", type=int, default=10)
    ap.add_argument("--stall_multiplier", type=int, default=30, help="multiplier for stall truncation")
    ap.add_argument("--hard_cap_multiplier", type=int, default=20, help="multiplier for hard cap")
    args = ap.parse_args()

    agent = A2CAgent.load(args.model)
