...
class DQNBaselineAgent:
    """
    DQN agent using original 11-feature representation.
    
    Architecture: input (11) -> hidden (128) tanh -> output (4)
    Same training loop as DQN-Extended, but with baseline 11 features.
    Used for diagnostic: if this works, extended features are broken.
    """

    IN = 11  # Original features baseline
    OUT = 4
    H = 128
    ...
    @classmethod
    def load(cls, path: str) -> "DQNBaselineAgent":
        z = np.load(path)
    ...
    def train_on_batch(self, batch, lr: float, gamma: float, target_net: Optional['DQNBaselineAgent'] = None):
        ...
        for obs, action, reward, next_obs, done in batch:
            ...
            if done:
                ...
                if target_net is not None:
                    ...
                    next_feat = compact_features(next_obs)
