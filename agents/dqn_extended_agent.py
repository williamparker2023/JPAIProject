...
class DQNExtendedAgent:
    """
    DQN agent using extended 21-feature representation.
    
    Architecture: input (21) -> hidden (128) tanh -> output (4)
    Same proven training loop as original DQN, but with richer features.
    """

    IN = 21  # Extended features instead of 11
    OUT = 4
    H = 128
    ...
    @classmethod
    def load(cls, path: str) -> "DQNExtendedAgent":
        z = np.load(path)
    ...
    def train_on_batch(self, batch, lr: float, gamma: float, target_net: Optional['DQNExtendedAgent'] = None):
        ...
        for obs, action, reward, next_obs, done in batch:
            ...
            if done:
                ...
                next_feat = extended_features(next_obs)
